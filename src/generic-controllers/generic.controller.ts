...
export interface IGenericRepository<T extends IGenericEntity>
...
>(props: {
  ...
  const modelSchema =
    '/dcic/signature-commons-schema/v5/core/' +
    props.modelName.toLowerCase() +
    '.json';
  ...
  class Controller {
    ...
    @get('/key_count', {
      ...
      responses: {
        '200': {
          description:
            props.modelName +
            ' model key_count (number of unique keys which appear in the query results)',
          content: {
            'application/json': {
              schema: {
                type: 'array',
    ...
    @get('/value_count', {
      ...
      responses: {
        '200': {
          description:
            props.modelName +
            ' model value_count (number of unique keys and values which appear in the query results)',
          content: {
            'application/json': {
              schema: {
                type: 'array',
    ...
    @get('/distinct_value_count', {
      ...
      responses: {
        '200': {
          description:
            props.modelName +
            ' model distinct_value_count (number of unique values which appear in the query results)',
          content: {
            'application/json': {
              schema: {
                type: 'array',
        ...
        '200': {
          ...
          content: {
            'application/json': {
              schema: {
                description:
                  'ajv errors experienced while validating database entries',
                type: 'array',
                items: {
                  type: 'object',
                },
        ...
        try {
          ...
          if (possibilities.length > 1)
            throw new HttpErrors.UnprocessableEntity(
              `Could not resolve single ${props.modelName} instance`,
            );
          else if (possibilities.length > 0) resolved_obj = possibilities[0];

          if (resolved_obj === undefined) {
            if (obj.id === undefined) (obj as any).id = uuidv4();
        ...
        '200': {
          ...
          content: {
            'application/json': {
              schema: {
                type: 'array',
                items: {'x-ts-type': props.GenericEntity},
              },
            },
          },
        },
    ...
    async find_get(
      ...
      return this.find({filter, contentRange});
    }
            ...
            schema: {
              ...
              properties: {
                ...
                contentRange: {
                  type: 'boolean',
                  description:
                    'whether or not to compute the contentRange header',
                },
              },
            },
          },
    ...
    async updateAll(
      @requestBody({
        description:
          'Object to replace all objects which match the `where` query with',
        required: true,
        content: {
          'application/json': {
            schema: {
      ...
      return this.genericRepository.updateAll(body, where);
    }
