...
export async function givenValidResourceData(
...
export async function givenInvalidResourceData(
  ctx: DatabaseTestContext,
  data?: Partial<Resource>,
) {
  const id = uuidv4();
...
export async function givenResource(
  ...
  return new ResourceRepository(await typeorm_db).create(
    <Partial<Resource>>await givenValidResourceData(ctx, data),
  );
}

export async function givenValidLibraryData(
  ctx: DatabaseTestContext,
  data?: Partial<Library>,
) {
  const id = uuidv4();
  const {meta, dataset, dataset_type} = libraryTest.tests.filter(
    test => test.valid,
  )[0].data;
...
export async function givenInvalidLibraryData(
  ctx: DatabaseTestContext,
  data?: Partial<Library>,
) {
  const id = uuidv4();
...
export async function givenLibrary(
  ctx: DatabaseTestContext,
  data?: Partial<Library>,
) {
  return new LibraryRepository(await typeorm_db).create(
    <Partial<Library>>await givenValidLibraryData(ctx, data),
  );
}

export async function givenValidSignatureData(
  ctx: DatabaseTestContext,
...
export async function givenInvalidSignatureData(
  ctx: DatabaseTestContext,
  data?: Partial<Signature>,
) {
  const id = uuidv4();
...
export async function givenSignature(
  ctx: DatabaseTestContext,
  data?: Partial<Signature>,
) {
  return new SignatureRepository(await typeorm_db).create(
    <Partial<Signature>>await givenValidSignatureData(ctx, data),
  );
}

export async function givenValidEntityData(
  ctx: DatabaseTestContext,
  data?: Partial<Entity>,
) {
  const id = uuidv4();
...
export async function givenInvalidEntityData(
  ctx: DatabaseTestContext,
  data?: Partial<Entity>,
) {
  const id = uuidv4();
...
export async function givenEntity(
  ctx: DatabaseTestContext,
  data?: Partial<Entity>,
) {
  return new EntityRepository(await typeorm_db).create(
    <Partial<Entity>>await givenValidEntityData(ctx, data),
  );
}

export async function givenValidSchemaData(
  ctx: DatabaseTestContext,
  data?: Partial<Schema>,
) {
  const id = uuidv4();
...
export async function givenInvalidSchemaData(
  ctx: DatabaseTestContext,
  data?: Partial<Schema>,
) {
  const id = uuidv4();
...
export async function givenSchema(
  ctx: DatabaseTestContext,
  data?: Partial<Schema>,
) {
  return new SchemaRepository(await typeorm_db).create(
    <Partial<Schema>>await givenValidSchemaData(ctx, data),
  );
}

export async function givenAdminUserProfileData(
  ctx: DatabaseTestContext,
...
export async function givenAdminUserProfile(
  ctx: DatabaseTestContext,
  data?: Partial<UserProfile>,
) {
  return new UserProfileRepository(await memory_db).create(
...
export async function givenEmptyDatabase(ctx: DatabaseTestContext) {
  ...
  await new SignatureRepository(await typeorm_db).deleteAll({
    id: {inq: ctx.signatures},
  });
  await new LibraryRepository(await typeorm_db).deleteAll({
    id: {inq: ctx.libraries},
  });
  await new ResourceRepository(await typeorm_db).deleteAll({
    id: {inq: ctx.resources},
  });
  await new SchemaRepository(await typeorm_db).deleteAll({
    id: {inq: ctx.schemas},
  });
